...
function NeuralPaths() {
    ...
    const { nodes, connections } = useMemo(() => {
        // Use a seeded approach for consistent server/client rendering
        const nodesList = Array.from({ length: 50 }, (_, i) => ({
            x: ((i * 137.508) % 800), // Pseudo-random but stable
            y: ((i * 214.718) % 600),
            id: `node-${i}`
        }));
        ...
        nodesList.forEach((node, i) => {
            const nearbyNodes = nodesList.filter((other, j) => {
                ...
                return distance < 120 && (i + j) % 3 === 0; // Stable condition instead of random
            });
...
export default function EnhancedBackgroundPaths({
    ...
    return (
        <div className="relative min-h-screen w-full overflow-hidden bg-white dark:bg-slate-950 flex items-center justify-center">
